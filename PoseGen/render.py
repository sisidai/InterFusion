...
obj_fname = '../data/smplx_uv/smplx_uv.obj'
_, _, t = nr.load_obj(obj_fname, load_texture=True, texture_size=8)


def render_single_batch(vertices, faces, angles, device):
    ...
    renderer = nr.Renderer(camera_mode='look_at', image_size=224).to(device)
    images_list = []
    for angle in angles:
