...
def add_index_to_image(in_path, out_path, index, position=(240, 440), color=(0, 0, 0), font_size=50):
    img = Image.open(in_path)
    draw = ImageDraw.Draw(img)
    
    font = ImageFont.truetype(fm.findfont(fm.FontProperties(family='DejaVu Sans')), font_size)
    draw.text(position, str(index), fill=color, font=font)
