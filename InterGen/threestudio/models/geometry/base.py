...
class BaseImplicitGeometry(BaseGeometry):
    ...
    def isocenter(self) -> Float[Tensor, "1 3"]:
        self._initilize_isosurface_helper()
        def batch_func(x):
            # scale to bbox as the input vertices are in [0, 1]
            field, _ = self.forward_field(
                scale_tensor(
                    x.to(self.bbox.device), self.isosurface_helper.points_range, self.bbox
                ),
            )
            field = field.to(
                x.device
        ...
        assert self.isosurface_helper is not None
        vertices_unscaled = self.isosurface_helper.grid_vertices
        field, _ = chunk_batch(
            batch_func,
            self.cfg.isosurface_chunk,
            vertices_unscaled,
        ...
        field_occ = 1 - torch.exp(-0.2 * field)
        field_occ = field_occ.clamp(min=0, max=1.1)
        vertices = scale_tensor(vertices_unscaled.to(self.bbox.device), self.isosurface_helper.points_range, self.bbox)
        return vertices[(field_occ>0.5).squeeze()].mean(dim=0)
