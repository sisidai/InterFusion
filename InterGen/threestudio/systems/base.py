...
class BaseLift3DSystem(BaseSystem):
    ...
    class Config(BaseSystem.Config):
        ...
        renderer_type_comp: str = ""
        renderer_comp: dict = field(default_factory=dict)

        guidance_type: str = ""
        guidance: dict = field(default_factory=dict)

        prompt_processor_type: str = ""
        prompt_processor_o: dict = field(default_factory=dict)
        prompt_processor_h: dict = field(default_factory=dict)
        prompt_processor_i: dict = field(default_factory=dict)
        prompt_processor_hh: dict = field(default_factory=dict)
    ...
    def configure(self) -> None:
        if (
            ...
            self.geometry_ = threestudio.find(self.cfg.geometry_type)(self.cfg.geometry)

        self.material = threestudio.find(self.cfg.material_type)(self.cfg.material)
        self.material_ = threestudio.find(self.cfg.material_type)(self.cfg.material)
        self.background = threestudio.find(self.cfg.background_type)(
            self.cfg.background
        )
        self.background_ = threestudio.find(self.cfg.background_type)(
            self.cfg.background
        )
        self.renderer_o = threestudio.find(self.cfg.renderer_type)(
            self.cfg.renderer,
            geometry=self.geometry,
            material=self.material,
            background=self.background,
        )
        self.renderer_h = threestudio.find(self.cfg.renderer_type)(
            self.cfg.renderer,
            geometry=self.geometry_,
            material=self.material_,
            background=self.background_,
        )
        self.renderer_i = threestudio.find(self.cfg.renderer_type_comp)(
            self.cfg.renderer_comp,
            geometry=self.geometry,
            material=self.material,
            background=self.background,
            geometry_=self.geometry_,
            material_=self.material_,
            background_=self.background_,
        )
